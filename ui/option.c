...
#define OPT_NOTIFY(...) do { if (!opt_silent) notifier_inform(__VA_ARGS__); } while (0)

//

static gboolean check_bool_val(GtkCheckButton *check, t_ent_bool *en, void (*update)(void)) { // NONNULL(1, 2, 3)
  // check is GtkCheckButton
  gboolean *pbool = EN_BOOLPTR(en);
  gboolean state = gtk_check_button_get_active(check);
  gboolean changed = pbool && (*pbool != state);
  if (changed) {
    *pbool = state;
    update();
  }
  ...
  if (en->prefix)
    OPT_NOTIFY("%s: %s: %s", en->prefix, name, enabled);
  else
    OPT_NOTIFY("%s: %s",                 name, enabled);
  return changed;
}
...
static void toggled_whois(void) {
  pingtab_vis_cols();
...
static void toggled_colors(void) {
  tab_dependent(NULL); style_set();
  tab_reload_theme();  drawtab_refresh();
  notifier_legend_refresh();
}

static void toggled_legend(void) {
  if (opts.graph)
    notifier_set_visible(NT_LEGEND_NDX, opts.legend);
  graphtab_refresh();
}

static void toggled_graph_params(void) {
  notifier_legend_vis_rows(-1);
  graphtab_refresh();
}

static void toggled_graph_area(void) {
  if (opts.legend)
    notifier_legend_update();
  graphtab_refresh();
}
...
#ifdef WITH_PLOT
static void toggled_pl_elems(void) {
  plottab_refresh(PL_PARAM_AT);
}

static void toggled_pl_rotor(void) {
  notifier_set_visible(NT_ROTOR_NDX, is_plelem_enabled(D3_ROTR));
  toggled_pl_elems();
}
#endif

typedef void (*toggled_bool_fn)(void); // GVoidFunc

static void toggle_cb(GtkCheckButton *check, t_ent_bool *en) {
  static toggled_bool_fn toggled_bool[] = {
    [ENT_BOOL_DNS]     = toggled_dns,
    [ENT_BOOL_HOST]    = pingtab_vis_cols,
    [ENT_BOOL_LOSS]    = pingtab_vis_cols,
    [ENT_BOOL_SENT]    = pingtab_vis_cols,
    [ENT_BOOL_RECV]    = pingtab_vis_cols,
    [ENT_BOOL_LAST]    = pingtab_vis_cols,
    [ENT_BOOL_BEST]    = pingtab_vis_cols,
    [ENT_BOOL_WRST]    = pingtab_vis_cols,
    [ENT_BOOL_AVRG]    = pingtab_vis_cols,
    [ENT_BOOL_JTTR]    = pingtab_vis_cols,
    [ENT_BOOL_AS]      = toggled_whois,
...
#ifdef WITH_PLOT
    [ENT_BOOL_PL_DARK] = toggled_colors,
#endif
    [ENT_BOOL_LGND]    = toggled_legend,
    [ENT_BOOL_AVJT]    = toggled_graph_params,
    [ENT_BOOL_CCAS]    = toggled_graph_params,
    [ENT_BOOL_LGHN]    = toggled_graph_params,
    [ENT_BOOL_MEAN]    = toggled_graph_area,
    [ENT_BOOL_JRNG]    = toggled_graph_area,
    [ENT_BOOL_AREA]    = toggled_graph_area,
#ifdef WITH_PLOT
    [ENT_BOOL_PLBK]    = toggled_pl_elems,
    [ENT_BOOL_PLAX]    = toggled_pl_elems,
    [ENT_BOOL_PLGR]    = toggled_pl_elems,
    [ENT_BOOL_PLRR]    = toggled_pl_rotor,
    [ENT_BOOL_GLOB]    = option_up_menu_ext,
#endif
  };
  if (GTK_IS_CHECK_BUTTON(check) && en) {
    int type = en->en.type;
    toggled_bool_fn fn = (type >= 0) && (type < (int)G_N_ELEMENTS(toggled_bool)) ?
      toggled_bool[type] : NULL;
    if (fn)
      check_bool_val(check, en, fn);
  }
}

static void radio_cb(GtkCheckButton *check, t_ent_rad_map *map) {
  if (!GTK_IS_CHECK_BUTTON(check) || !map)
    return;
  uint ndx = map->ndx;
  if (ndx >= G_N_ELEMENTS(ent_rad))
    return;
    ...
    if (selected && (map->val != *pval)) {
      *en->pval = map->val;
      if (en->cb)
        en->cb();
      if (map->str)
        OPT_NOTIFY("%s: %s", en->c.en.name, map->str);
      else
        OPT_NOTIFY("%s: %d", en->c.en.name, map->val);
    }
  }
}
...
static void arrow_cb(GtkToggleButton *self G_GNUC_UNUSED, t_ent_exp_common *en) {
  if (en && GTK_IS_TOGGLE_BUTTON(en->arrow)) {
    ...
    if (GTK_IS_WIDGET(en->sub))
      gtk_widget_set_visible(en->sub, active);
  ...
  switch (en->en.type) {
    ...
    case ENT_STR_PSIZE: {
      ...
      if (okay) {
        if (en->pint) {
          int n = *en->pint;
          if (en->en.type == ENT_STR_IVAL) // extra
            opts.tout_usec = n * G_USEC_PER_SEC;
          OPT_NOTIFY("%s: %d", en->en.name, n);
        }
      } else
        set_ed_texthint(en);
      if (en->en.type == ENT_STR_IVAL)
        drawtab_refresh(); // rescale time-axis
    } break;
    case ENT_STR_PLOAD: {
      char *pad = parser_str(got, en->en.name, OPT_TYPE_PAD);
      if (pad) {
        if (en->pstr)
          g_strlcpy(en->pstr, pad, en->slen);
...
static void minttl_cb(GtkSpinButton *spin, t_ent_spn_elem *en) {
  if (!GTK_IS_SPIN_BUTTON(spin) || !en)
    return;
  int *pmin = en->aux[0].pval;
  if (!pmin)
    return;
...
static void colgrad_cb(GtkSpinButton *spin, t_ent_spn_aux *aux) {
  if (!GTK_IS_SPIN_BUTTON(spin) || !aux)
    return;
  int *pval = aux->pval;
  if (!pval)
    return;
  ...
  if ((minmax->min <= got) && (got <= minmax->max)) {
    *pval = got;
    ...
    WARN("%s [%d,%d]: %d", OUTRANGE_ERR, minmax->min, minmax->max, got);
}
      ...
      *pval = want; t_ent_spn_elem *list = ent_spn[aux->sn].list;
      if (list) for (int i = 0; i < ENT_SPN_ANGS; i++) { // adjust angle entries' step
        ...
        if (GTK_IS_SPIN_BUTTON(spin))
          gtk_spin_button_set_increments(spin, want, want * 6);
      }
    } else {
      *pval = want;
      set_rotor_n_redraw(step, aux->rev, aux->pn);
    }
  }
}
...
static void scalefov_cb(GtkSpinButton *spin, t_ent_spn_aux *aux) {
  ...
  if ((*pval == got) || (got < minmax->min) || (got > minmax->max))
    return;
  *pval = got;
...
static GtkWidget* label_box(const char *name, const char *unit) {
  ...
  char *nameunit = unit ? g_strdup_printf("%s, %s", name, unit) : NULL;
  GtkWidget *label = gtk_label_new(nameunit ? nameunit : name);
  g_free(nameunit);
  g_return_val_if_fail(GTK_IS_LABEL(label), box);
  gtk_label_set_use_underline(GTK_LABEL(label), true);
  gtk_box_append(GTK_BOX(box), label);
  if (style_loaded)
    gtk_widget_add_css_class(label, CSS_PAD);
...
static GtkWidget* add_labelbox(GtkWidget* list, const char *name, const char *unit) {
  // list is either GtkBox or GtkListBox
  GtkWidget *box = label_box(name, unit);
  if (box) {
    if (GTK_IS_LIST_BOX(list))
      gtk_list_box_append(GTK_LIST_BOX(list), box);
    else if (GTK_IS_BOX(list))
      gtk_box_append(GTK_BOX(list), box);
  }
  return box;
}

static GtkWidget* add_opt_check(GtkWidget* list, t_ent_bool *en) { // NONNULL(1, 2)
  // list is either GtkBox or GtkListBox
  ...
  if (box) {
    ...
    if (style_loaded)
      gtk_widget_add_css_class(check, CSS_RPAD);
    gtk_widget_set_halign(check, GTK_ALIGN_END);
    g_signal_connect(check, EV_TOGGLE, G_CALLBACK(toggle_cb), en);
    en->check = GTK_CHECK_BUTTON(check);
    gboolean *pbool = EN_BOOLPTR(en);
    if (pbool)
      gtk_check_button_set_active(en->check, *pbool);
  }
  return box;
}

static GtkWidget* add_opt_enter(GtkWidget* list, t_ent_str *en) { // NONNULL(1, 2)
  // list is GtkListBox
  GtkWidget *box = add_labelbox(list, en->en.name, en->en.unit);
  if (box) {
    en->box = box;
    ...
    if (style_loaded)
      gtk_widget_add_css_class(en->input, CSS_NOPAD);
...
static GtkWidget* add_expand_common(GtkWidget* list, t_ent_exp_common *en) { // NONNULL(1, 2)
  // list is GtkListBox
...
static void add_opt_exp_entry(GtkWidget *list, t_elem_desc *desc) { // NONNULL(1, 2)
  // list is GtkListBox
  type2ent_fn type2ent = desc->t2e;
  if (type2ent) for (int i = desc->mm.min; i <= desc->mm.max; i++) {
    int type = desc->elems[i].type;
    int ndx = type2ent(type);
    if (ndx < 0)
      WARN("%s: %d", UNKNTYPE_HDR, type);
    else
      add_opt_check(list, &ent_bool[ndx]);
  }
}

static GtkWidget* add_opt_expand(GtkWidget* list, t_ent_exp ens[], uint en_len) { // NONNULL(1, 2)
  // list is GtkListBox
  if (en_len < 1)
    return NULL;
  GtkWidget *box = add_expand_common(list, &ens->c);
  if (GTK_IS_BOX(box)) {
    GtkWidget *sub = ens->c.sub;
    if (sub) {
      t_ent_exp *en = ens;
      for (uint i = 0; i < en_len; i++, en++)
        if (en->desc)
          add_opt_exp_entry(sub, en->desc);
    }
  }
  return box;
}
...
static gboolean add_minmax(GtkWidget *box, t_ent_spn_elem *en, uint ndx, const int *step, gboolean wrap) {
  gboolean okay = false;
  if (GTK_IS_BOX(box) && en) {
    ...
    if (mm0 && mm1) {
      ...
      if (GTK_IS_SPIN_BUTTON(spin)) {
        ...
        g_warning("%s: %s", ERROR_HDR, "gtk_spin_button_new_with_range()");
    }
  }
  return okay;
}
...
static GtkWidget* add_opt_range(GtkWidget* list, t_ent_spn *en) { // NONNULL(1, 2)
  // list is GtkListBox
  GtkWidget *box = add_expand_common(list, &en->c);
  if (GTK_IS_BOX(box) && en->c.sub) {
    t_ent_spn_elem *elem = en->list;
    for (uint n = 0; (n < G_N_ELEMENTS(en->list)) && elem && elem->title; n++, elem++) {
      ...
      switch (elem->kind) {
        ...
        case ROTOR_COLUMN:
          ...
          for (uint i = 0; i < ENT_SPN_ROT_MAX; i++) if (elem->aux[i].prfx) {
            ...
            if (/*(i >= ENT_SPN_ROT0) &&*/ i <= ENT_SPN_ROT2)
              // leave only buttons for rotation spins
              for (GtkWidget *txt = gtk_widget_get_first_child(GTK_WIDGET(elem->aux[i].spin));
                   GTK_IS_WIDGET(txt);
                   txt = gtk_widget_get_next_sibling(txt))
                if (GTK_IS_TEXT(txt))
                  gtk_widget_set_visible(txt, false);
          }
          break;
        case SCALE_SPIN:
...
static GtkWidget* add_opt_radio(GtkWidget* list, t_ent_rad *en) { // NONNULL(1, 2)
  // list is GtkListBox
  if (!en)
    return NULL;
  GtkWidget *box = add_expand_common(list, &en->c);
...
#else
  for (GtkWidget *c; (c = gtk_widget_get_first_child(GTK_WIDGET(box)));)
...
static gboolean create_optmenu(GtkWidget *bar, const char **icons, const char *tooltip, gboolean (*fn)(GtkWidget*)) {
  ...
  if (ico)
    gtk_menu_button_set_icon_name(GTK_MENU_BUTTON(menu), ico);
  else
    g_warning("%s: %s", OPTIONS_HDR, NOICON_ERR);
  if (tooltip)
  ...
  if (GTK_IS_POPOVER(popover)) {
    ...
    okay = false;
  if (!okay)
    gtk_header_bar_remove(GTK_HEADER_BAR(bar), menu);
  return okay;
}

#define EN_PR_FMT(ndx, fmt, arg) do {                               \
  snprintg(ent_str[ndx].hint, sizeof(ent_str[ndx].hint), fmt, arg); \
  if (!add_opt_enter(list, &ent_str[ndx]))                          \
    okay = false;                                                   \
} while (0)
...
static gboolean create_main_optmenu(GtkWidget *list) {
  ...
  if (list)
    main_optmenu_list = list;
  else
    list = main_optmenu_list;
...
static gboolean create_ext_optmenu(GtkWidget *list) {
  ...
  if (list)
    ext_optmenu_list = list;
  else
    list = ext_optmenu_list;
  g_return_val_if_fail(GTK_IS_LIST_BOX(list), false);
  gboolean okay = true;
  gtk_compat_list_box_remove_all(GTK_LIST_BOX(list));
  ...
  if (opts.graph) {
    ...
    if (!add_opt_expand(list,   &ent_exp[ENT_EXP_LGFL], 1))   okay = false;
    if (!add_opt_expand(list,   &ent_exp[ENT_EXP_GREX], 1))   okay = false;
#ifdef WITH_PLOT
    if (opts.plot) {
      if (!add_opt_expand(list, &ent_exp[ENT_EXP_PLEL], 1))   okay = false;
...
#define ENT_LOOP_SET(row) do {             \
  if (GTK_IS_WIDGET(row))                  \
    gtk_widget_set_sensitive(row, notrun); \
} while (0)
#define ENT_LOOP_STR(arr) do {                 \
  for (uint i = 0; i < G_N_ELEMENTS(arr); i++) \
