...
static int  msrcelem_type2ndx(int type) { return elem_type2ndx(type, msrcelem,  G_N_ELEMENTS(msrcelem));  }
...
static int msrcelem_type2ent(int type) { // only one element
  return (type == WR_MS) ? ENT_BOOL_MSRC : -1;
}
...
#define T_ELEM_DESC(_elems, _cat, _patt, _min, _max) {             \
  .elems = _elems, .len = G_N_ELEMENTS(_elems),                    \
  .cat = _cat, .patt = _patt, .mm = { .min = _min, .max = _max },  \
  .t2n = _elems ## _type2ndx, .t2e = _elems ## _type2ent,          \
}
t_elem_desc info_desc = T_ELEM_DESC(pingelem,  INFO_CHAR, INFO_PATT, PE_HOST, PE_RT  );
t_elem_desc msrc_desc = T_ELEM_DESC(msrcelem,  0,         0,         WR_MS,   WR_MS  );
t_elem_desc stat_desc = T_ELEM_DESC(pingelem,  STAT_CHAR, STAT_PATT, PE_LOSS, PE_JTTR);
t_elem_desc grlg_desc = T_ELEM_DESC(graphelem, GRLG_CHAR, GRLG_PATT, GE_AVJT, GE_LGHN);
t_elem_desc grex_desc = T_ELEM_DESC(graphelem, GREX_CHAR, GREX_PATT, GX_MEAN, GX_AREA);
#ifdef WITH_PLOT
t_elem_desc plot_desc = T_ELEM_DESC(plotelem,  PLEL_CHAR, PLEL_PATT, D3_BACK, D3_ROTR);
#endif
#undef T_ELEM_DESC
...
#define INIT_WR_NT(ndx, ename, etip) INIT_XE_NT(msrcelem, (ndx), (ename), (etip))
  //
  ...
  INIT_WR_NT(WR_MS,   ELEM_MSRC_HDR, ELEM_MSRC_TIP);
...
void clean_elems(int type) {
  switch (type) {
    ...
    case ENT_EXP_MSRC: clean_type_elems(msrcelem,  G_N_ELEMENTS(msrcelem),  WR_MS,   WR_MS);   break;
